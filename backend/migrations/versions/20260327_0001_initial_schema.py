...
organization_status = sa.Enum('active', 'suspended', 'deleted', name='organizationstatus')
organization_plan_type = sa.Enum('starter', 'growth', 'enterprise', name='organizationplantype')
role_scope = sa.Enum('global', 'org', name='rolescope')
user_status = sa.Enum('active', 'invited', 'disabled', name='userstatus')
vehicle_type = sa.Enum('railcar', 'container', name='vehicletype')
load_type = sa.Enum('cylinder', 'cube', name='loadtype')
load_session_status = sa.Enum('draft', 'optimized', 'failed', name='loadsessionstatus')
optimization_status = sa.Enum('pending', 'running', 'completed', 'failed', name='optimizationstatus')
metric_type = sa.Enum('cpu_usage', 'memory_usage', 'request_count', 'error_count', 'job_latency', name='metrictype')


def upgrade() -> None:
    bind = op.get_bind()
    organization_status.create(bind, checkfirst=True)
    organization_plan_type.create(bind, checkfirst=True)
    role_scope.create(bind, checkfirst=True)
    user_status.create(bind, checkfirst=True)
    vehicle_type.create(bind, checkfirst=True)
    load_type.create(bind, checkfirst=True)
    load_session_status.create(bind, checkfirst=True)
...
def downgrade() -> None:
    ...
    load_session_status.drop(bind, checkfirst=True)
    load_type.drop(bind, checkfirst=True)
    vehicle_type.drop(bind, checkfirst=True)
    user_status.drop(bind, checkfirst=True)
    role_scope.drop(bind, checkfirst=True)
    organization_plan_type.drop(bind, checkfirst=True)
