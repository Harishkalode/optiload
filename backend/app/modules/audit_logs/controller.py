...
@router.get("")
def list_audit_logs(user_id: int | None = None, action: str | None = None, db: Session = Depends(get_db),
    ...
    return success_response([{"id": l.id, "user_id": l.user_id, "organization_id": l.organization_id,
                              "action": l.action, "resource": l.resource, "resource_id": l.resource_id,
                              "metadata_json": l.metadata_json, "ip_address": l.ip_address, "timestamp": l.timestamp}
                             for l in logs])
...
def get_audit_log(log_id: int, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    ...
    return success_response(
        {"id": log.id, "user_id": log.user_id, "organization_id": log.organization_id, "action": log.action,
         "resource": log.resource, "resource_id": log.resource_id, "metadata_json": log.metadata_json,
