...
@router.get("")
def list_vehicles(page: int = Query(default=1, ge=1), page_size: int = Query(default=20, ge=1, le=100), type: str | None = None, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    ...
    return success_response({"items": [{"id": v.id, "organization_id": v.organization_id, "type": v.type.value, "dimensions": v.dimensions, "capacity": v.capacity} for v in sliced], "total": len(vehicles), "page": page, "page_size": page_size})
...
def get_vehicle(vehicle_id: int, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    ...
    return success_response({"id": v.id, "organization_id": v.organization_id, "type": v.type.value, "dimensions": v.dimensions, "capacity": v.capacity})
...
def update_vehicle(vehicle_id: int, payload: VehicleUpdateRequest, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    org_id = get_tenant_organization_id(current_user)
    if org_id is None:
        raise AppError("ORG_REQUIRED", "organization context is required")
