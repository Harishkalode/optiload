...
import {
  ...
  Zap, CheckSquare, Square, AlertTriangle, ToggleLeft, ToggleRight, RefreshCw,
} from 'lucide-react';
...
const CONSTRAINTS = [
  { key: 'weightBalance', label: 'Weight Distribution Balancing', desc: 'Optimize axle load distribution', default: true },
  { key: 'stackRules', label: 'Enforce Stack Rules', desc: 'Apply fragility and stackability constraints', default: true },
  { key: 'hazmatSeparation', label: 'Hazmat Separation', desc: 'Keep hazardous materials isolated', default: false },
  { key: 'priorityOrder', label: 'Priority-Based Loading', desc: 'Load high-priority items first', default: true },
];
...
export function OptimizationJobs() {
  ...
  const [selectedLoads, setSelectedLoads] = useState<Map<number, number>>(new Map());
  const [selectedLoadDetail, setSelectedLoadDetail] = useState<LoadRow | null>(null);
  ...
  const loadData = useCallback(async (silent = false) => {
    if (!silent) setListLoading(true); else setRefreshing(true);
    setListError(null);
    try {
      const [v, l] = await Promise.all([fetchVehicles(1, 200), fetchLoads(1, 200)]);
      const vItems = (v as any).items ?? (Array.isArray(v) ? v : []);
      const lItems = (l as any).items ?? (Array.isArray(l) ? l : []);
      setVehicles(vItems);
      setLoads(lItems);
    } catch (e) {
      setListError(e instanceof Error ? e.message : 'Failed to load fleet data');
    } finally {
      if (!silent) setListLoading(false); else setRefreshing(false);
    }
  }, []);
  ...
  const totalWeight = useMemo(
    () => loads.reduce((a, l) => a + l.weight * (selectedLoads.get(l.id) || 0), 0),
    [loads, selectedLoads],
  );

  const selectedVehicle = vehicles.find(v => v.id === selectedVehicleId) ?? null;

  const toggleLoad = (load: LoadRow) => {
    setSelectedLoads(prev => {
      const next = new Map(prev);
      if (next.has(load.id)) {
        next.delete(load.id);
        if (selectedLoadDetail?.id === load.id) setSelectedLoadDetail(null);
      } else {
        next.set(load.id, 1);
  ...
  const updateLoadQty = (loadId: number, qty: number) => {
    if (qty < 1) return;
    setSelectedLoads(prev => {
      const next = new Map(prev);
      next.set(loadId, qty);
  ...
  const handleRun = async () => {
    ...
    setRunning(true);
    try {
      const loadsPayload = [...selectedLoads.entries()].map(([loadId, quantity]) => ({ load_id: loadId, quantity }));
      const res = await runOptimization(selectedVehicleId, loadsPayload, {
        weight_balance: constraints.weightBalance,
        stack_rules: constraints.stackRules,
        hazmat_separation: constraints.hazmatSeparation,
        priority_order: constraints.priorityOrder,
      });
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-4">
        <h2 style={{ fontFamily: 'Space Grotesk, sans-serif', fontWeight: 700, fontSize: 18, color: textPrimary }}>New Optimization Job</h2>
        <button type="button" className="flex items-center gap-2 rounded-lg px-3 py-2 transition-all"
          style={{ background: isDark ? '#161D2A' : '#F8FAFC', border: `1px solid ${border}`, color: text, fontSize: 13, cursor: 'pointer' }}
      ...
      <OLCard padding="20px" style={{ marginBottom: 24 }}>
        <div className="flex items-center">
          {STEPS.flatMap((s, i) => {
            ...
            if (i < STEPS.length - 1) {
              items.push(<div key={`connector-${s.id}`} className="flex-1 mx-4" style={{ height: 2, background: step > s.id + 1 ? palette.primary : border, transition: 'background 0.3s' }} />);
            }
            return items;
          })}
        </div>
          ...
          <div className="xl:col-span-2">
            <OLCard padding="0">
              <div className="p-4" style={{ borderBottom: `1px solid ${border}` }}>
                <div style={{ fontFamily: 'Space Grotesk, sans-serif', fontWeight: 600, fontSize: '14px', color: textPrimary }}>Vehicles</div>
                <div style={{ fontSize: '12px', color: text, marginTop: 2 }}>Select exactly one vehicle</div>
              </div>
              {vehicles.length === 0 ? (
      ...
      {step === 2 && (
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          <div className="xl:col-span-2">
            <OLCard padding="0">
              <div className="p-4" style={{ borderBottom: `1px solid ${border}` }}>
                <div style={{ fontFamily: 'Space Grotesk, sans-serif', fontWeight: 600, fontSize: '14px', color: textPrimary }}>Loads</div>
              ...
              {loads.length === 0 ? (
                ...
                <table className="w-full" style={{ borderCollapse: 'collapse' }}>
                  <thead>
                    <tr style={{ borderBottom: `1px solid ${border}` }}>
                      <th className="px-4 py-3 w-10" />
                      {['ID', 'Type', 'Weight', 'Dimensions'].map(h => (
                        <th key={h} className="text-left px-4 py-3" style={{ fontSize: '11px', fontWeight: 600, color: text, letterSpacing: '0.04em', textTransform: 'uppercase' }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {loads.map(l => {
                      const sel = selectedLoads.has(l.id);
                      return (
                        <tr key={l.id}
                          onClick={() => { toggleLoad(l); if (!sel) setSelectedLoadDetail(l); }}
                          style={{ borderBottom: `1px solid ${border}`, cursor: 'pointer', background: sel ? `${palette.primary}10` : 'transparent' }}
                          onMouseEnter={e => { if (!sel) e.currentTarget.style.background = rowHover; }}
                          onMouseLeave={e => { if (!sel) e.currentTarget.style.background = 'transparent'; }}
                          className="transition-colors">
                          ...
                          <td className="px-4 py-3" style={{ fontSize: '12px', color: textPrimary, fontFamily: 'JetBrains Mono, monospace' }}>{l.weight.toLocaleString()} kg</td>
          ...
          {/* Right panel: Load details + quantity */}
          <div>
            <OLCard padding="20px">
              <div style={{ fontFamily: 'Space Grotesk, sans-serif', fontWeight: 600, fontSize: '14px', color: textPrimary, marginBottom: 16 }}>Load Details</div>
              {!selectedLoadDetail ? (
                <div className="flex flex-col items-center py-8" style={{ color: text }}>
                  <Package size={32} style={{ opacity: 0.3, marginBottom: 8 }} />
                  <span style={{ fontSize: '12px' }}>Select a load to view details</span>
                </div>
              ) : (
                <div className="space-y-4">
                  <div className="p-3 rounded-lg" style={{ background: isDark ? '#161D2A' : '#F8FAFC', border: `1px solid ${border}` }}>
                    <div style={{ fontSize: '12px', fontWeight: 600, color: textPrimary, marginBottom: 4 }}>#{selectedLoadDetail.id} — {selectedLoadDetail.type}</div>
                    <div style={{ fontSize: '11px', color: text, marginBottom: 8 }}>{fmtDims(selectedLoadDetail.dimensions)}</div>
                    <div className="space-y-2">
                      {[
                        ['Weight', `${selectedLoadDetail.weight.toLocaleString()} kg`],
                        ['Fragile', selectedLoadDetail.fragile ? 'Yes' : 'No'],
                        ['Stackable', selectedLoadDetail.stackable ? 'Yes' : 'No'],
                        ['HAZMAT', selectedLoadDetail.hazmat_class || 'None'],
                      ].map(([label, value]) => (
                        <div key={label} className="flex justify-between">
                          <span style={{ fontSize: '11px', color: text }}>{label}</span>
                          <span style={{ fontSize: '11px', color: textPrimary, fontWeight: 500 }}>{value}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                  <div>
                    <label style={{ fontSize: '12px', fontWeight: 600, color: textPrimary, display: 'block', marginBottom: 6 }}>Quantity</label>
                    <div className="flex items-center gap-2">
                      <button onClick={() => updateLoadQty(selectedLoadDetail.id, (selectedLoads.get(selectedLoadDetail.id) || 1) - 1)}
                        className="w-8 h-8 rounded-lg flex items-center justify-center"
                        style={{ background: isDark ? '#1E2A38' : '#E2E8F0', color: textPrimary, fontSize: '16px' }}>−</button>
                      <input type="number" min={1} value={selectedLoads.get(selectedLoadDetail.id) || 1}
                        onChange={e => updateLoadQty(selectedLoadDetail.id, Math.max(1, parseInt(e.target.value) || 1))}
                        style={{ width: 60, textAlign: 'center', background: inputBg, border: `1px solid ${border}`, color: textPrimary, borderRadius: 8, padding: '6px 8px', fontSize: '14px', fontFamily: 'JetBrains Mono, monospace' }} />
                      <button onClick={() => updateLoadQty(selectedLoadDetail.id, (selectedLoads.get(selectedLoadDetail.id) || 1) + 1)}
                        className="w-8 h-8 rounded-lg flex items-center justify-center"
                        style={{ background: isDark ? '#1E2A38' : '#E2E8F0', color: textPrimary, fontSize: '16px' }}>+</button>
                    </div>
      ...
      {step === 3 && (
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          <div className="xl:col-span-2">
            <OLCard padding="20px">
              <div style={{ fontFamily: 'Space Grotesk, sans-serif', fontWeight: 600, fontSize: '14px', color: textPrimary, marginBottom: 20 }}>Constraint preferences</div>
              <div className="space-y-4">
                {CONSTRAINTS.map(c => (
            ...
            <OLCard padding="20px">
              ...
              <div className="space-y-3">
                {[
                  ...
                  { label: 'Loads', value: `${selectedLoads.size} types` },
                  { label: 'Total items', value: `${[...selectedLoads.values()].reduce((a, b) => a + b, 0)}` },
      ...
      <div className="flex items-center justify-between mt-6">
        <OLButton variant="secondary" size="md" icon={<ChevronLeft size={16} />} onClick={() => step > 1 && setStep(step - 1)} disabled={step === 1}>Previous</OLButton>
        {step < 3 && (
          <OLButton variant="primary" size="md" iconRight={<ChevronRight size={16} />} onClick={() => setStep(step + 1)}>Next step</OLButton>
        )}
      </div>
    </div>
  );
